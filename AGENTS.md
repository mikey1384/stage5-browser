...
- Before clicking a fresh observed ref that is outside the viewport, use a bounded native scroll-into-view step, then revalidate uniqueness, attachment, visibility, enabled state, viewport intersection, and pointer interception. Consume the snapshot on every attempted ref click. If viewport preparation fails, return immediately with `actionDispatched: false`; do not enter the ambiguous click path.
