...
- Instrument exact-ref pointer dispatch with bounded, privacy-safe event evidence. A normal stable-click timeout may use one guarded forced exact-handle attempt only when the first attempt emitted no trusted event and the same node remains fully actionable. Block events that miss the exact node or reach it after its state changes. Never force after partial input, unavailable evidence, detachment, or ambiguity.
