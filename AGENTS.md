...
- Count loading indicators only within the visible portion of the selected scroll surface. For document scrolling, prefer one uniquely visible semantic `[role="feed"]`; otherwise use the current viewport. Unrelated offscreen or out-of-feed loaders must not hold a content wait open.
