...
- Instrument exact-ref pointer dispatch with bounded, privacy-safe event evidence. Activate and verify the controller-selected page immediately before input. If a controlled Chromium page remains browser-hidden on macOS, resolve its exact target window, restore it when minimized, foreground only the verified Stage5-owned process, then re-check renderer visibility; never select a process by name/title or expose its PID/window ID/geometry. A normal stable-click timeout may use one guarded forced exact-handle attempt only when the first attempt emitted no trusted event and the same node remains fully actionable. If both handle paths emit zero events, one page-level mouse dispatch may target the exact fresh main-frame hit point while the same guard remains live. Block events that miss the exact node or reach it after its state changes. Never fall back after partial input, unavailable evidence, detachment, cross-frame ambiguity, or failed page activation; never expose or journal coordinates.
