...
11. Use `browser_scroll` to advance infinite timelines and `browser_find_text` to search what is currently rendered. If document scrolling does not move and reports nested candidates, take a fresh snapshot and pass the intended `scrollContainers` capability through `target`; never guess a selector. Use `waitFor` with `article_count_growth`, `loading_indicators_disappear`, or `either` when a feed is showing loaders. `targetBoundaryReached: true` is only geometry, calculated with one-CSS-pixel tolerance; `documentBoundaryReached` is true only for the frame document. A downward feed ends only when `endReached: true`, normally through a caller-supplied visible `endMarker`; `dynamic_content_stalled`, a timed-out content wait, and `geometric_boundary_unconfirmed` require inspection rather than a completeness claim. Use diagnostics to inspect the sanitized network activity correlated to the last scroll.
