...
A later read-only observation found that the card still displayed its processing state after more than 60 seconds. The correlated GraphQL request returned HTTP 200 and no HTTP error was observed, but transport success is not proof that Facebook finished generating the preview. This does not change the fill result: `browser_fill_ref` proves only the editor-value boundary, not downstream application processing. The safe terminal state is therefore "draft filled; link-preview processing unconfirmed." The draft must not be filled again, and Next/Post must remain untouched unless a fresh site-visible state proves readiness and the user separately authorizes continuation.
