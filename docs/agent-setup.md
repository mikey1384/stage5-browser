...
Release 0.11.0 adds exact opaque tab identities plus non-activating `browser_inspect_tab`, and changes `browser_select_tab` to require a fresh `tabId`. The surface is 32 tools, tool catalog 10, and worker protocol 8. Tasks using an older contract require one host reconnect. Do not reinstall or duplicate the existing `stage5_browser` registration. ChatGPT/Codex should reconnect its MCP host once; Claude Code should exit and resume the same conversation with `claude --continue`.
