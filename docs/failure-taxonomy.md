...
| Active-page drift | an auxiliary popup appears, the selected page closes, or the context briefly reports no visible page | explicit controller selection, live-page membership, unique handoff visibility, and sole-page fallback | worker | preserve a valid selected page; never promote an auxiliary page implicitly; when exactly one live page remains, report it as active rather than returning a null index |
