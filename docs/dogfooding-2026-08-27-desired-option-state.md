...
Historical operations remain non-retriable. A release notice or Lounge message does not authorize passive inspection, correction, form continuation, submission, or any account action. Any live resume begins from fresh direct controlling-thread authority and a new exact inspection. Deselect only an option reported `selected:true`; stop on null or conflicting state.
