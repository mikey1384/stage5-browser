...
Agent browser operations are unavailable while the native process owns the profile. Resume refuses a running process, remaining profile lock, launch-identity mismatch, or a caller-supplied non-root post-login route that cannot be reached after the human phase added target-origin session metadata. Current-session process evidence governs the shutdown gate: exit code zero plus no signal and zero locks permits reattachment. Chromium's stored exit type and `exited_cleanly` flag are advisory; Stage5 compares their value and Preferences modification time with the pre-handoff snapshot so a stale `crashed` marker is reported without being blamed on the current session. If the process exit is abnormal or unavailable but the process is gone and locks are clear, the first resume reports a bounded override and one deliberate second resume reattaches the same profile. It never kills the human process, deletes locks, edits exit preferences, or selects cookie values.

Once allowed, the controller captures three target-origin storage checkpoints: offline metadata immediately after native exit, live context metadata immediately after Playwright starts and before explicit target navigation, and live context metadata after the target loads. It also records whether the target was already restored at controlled start, the initial `navigator.webdriver` observation, and the actual Chromium Profile Path obtained from a live Chromium diagnostics surface. Filesystem aliases are canonicalized for comparison while both configured and observed paths remain visible. Cookie keys are hashed only for in-process set comparison and values are never read or returned. The resulting `lossBoundary` distinguishes launch-time loss, an ambiguous launch/restored-target boundary, target-load loss, preservation, and insufficient evidence. `automationCorrelation` reports co-occurrence only; it is not a causal conclusion. The controller then removes the marker tab and returns a bounded semantic preview. A fresh full semantic snapshot remains required to prove site-specific signed-in state. Exact human clicks and the uncontrolled window's network traffic are intentionally unobservable.
