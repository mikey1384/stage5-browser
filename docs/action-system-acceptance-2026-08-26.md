...
| --- | --- | --- | --- |
| Two role-free option groups were represented by one broad positioned portal, so passive inspection found one surface with multiple plausible owners | Bounded discovery partitions only strict option branches with a real visual gap below the shared portal, retains an inner scroll surface, and keeps contiguous wrappers coalesced. Ownership remains explicit/structural/focused/expanded/spatial; unresolved layouts remain zero-input. | Separate-portal, broad-portal partition, contiguous-wrapper, and equal-anchor cases in `tests/browser-controller/core/control-reveal-recovery.test.ts` | Reproduced from 0.15.5 trace and fixed in compatible 0.15.6 candidate; reporting page remains frozen until release adoption |
...
The additive 0.15.5 and 0.15.6 evidence is recorded separately in `docs/dogfooding-2026-08-26-positioned-popup-and-hit-test-evidence.md` and `docs/dogfooding-2026-08-26-popup-partition-and-all-hidden-proof.md`; it does not replace or reinterpret the historical 0.13 gate above.
