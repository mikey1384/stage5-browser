...
| --- | --- | --- | --- |
| One role-free surface had a uniquely nearest opener plus a farther control that still passed the broad spatial plausibility threshold | Ownership preserves explicit/structural, focused, expanded, and spatial tiers. Within a non-structural tier, normalized edge gap may resolve only one candidate with a decisive lead; structural conflicts and exact/near ties remain zero-input ambiguous. Portal partitioning still requires a real branch gap and keeps contiguous wrappers coalesced. | Separate/shared portal, contiguous wrapper, uniquely nearest anchor, and exact positional tie cases in `tests/browser-controller/core/control-reveal-recovery.test.ts` | 0.15.6 live trace ruled out branch partitioning; binary spatial ambiguity reproduced and fixed in compatible 0.15.7 candidate |
...
The additive 0.15.5–0.15.7 evidence is recorded in the corresponding `docs/dogfooding-2026-08-26-*` records; it does not replace or reinterpret the historical 0.13 gate above.
