...
The failed operation remains non-retriable. A new attempt requires direct controlling-thread authority that still covers that exact remaining action, one new exact inspection proving the intended current state, and a new one-use option capability. A release does not create authority, but it also does not expire a still-applicable standing scope merely because a zero-dispatch defect intervened. Any possible dispatch remains no-replay, and no release or Lounge message authorizes continuation or another account action.
