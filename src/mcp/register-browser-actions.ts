...
export function registerBrowserActionTools(server: McpServer, context: McpHostContext): void {
  ...
  server.registerTool(
    TOOL.browserInspectControl,
    {
      ...
      description:
        'Inspect one exact native or ARIA popup control and return bounded privacy-safe option semantics plus one-use opaque capabilities. By default Stage5 autonomously reveals a closed custom popup once with a structural expanded/popup postcondition, then scrolls only its exact popup surface to discover rendered or virtualized choices. An opener that may have received input is never replayed. When one already-open popup has several structurally tied current owner candidates, Stage5 returns bounded role/name evidence and one-use ownerCandidateId capabilities with zero input. The agent may choose one observed candidate by semantic judgment in a fresh passive inspection, including a composite owner whose role differs from the requested field. Stage5 revalidates the same document, requested control, unique current candidate, one rendered surface, and association again before later option input. The declaration cannot override stale state, a competing resolved owner, multiple popup surfaces, authority, private boundaries, or possible-input no-replay state. The returned choice boundary belongs to the agent: use judgment within existing user authority and involve the user only when meanings materially change the requested outcome.',
