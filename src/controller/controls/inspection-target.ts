...
import {
  observeOpenPopupOwnerCandidates,
  type ControlPopupAssociation,
} from './popup-association.js';
...
export async function resolveInspectionTarget(
  ...
  const agentDeclaredPopupOwner = context.consumeAgentDeclaredPopupOwner(
    frame,
    documentVersion,
    input.control,
  ...
  try {
    ...
    }, false, 'control_missing_with_open_popup');
  }
}
...
function controlUnavailableBeforeInspection(error: unknown): error is Stage5BrowserError {
  return error instanceof Stage5BrowserError &&
    error.code === 'TARGET_NOT_FOUND' &&
    (error.details?.reason === 'control_not_found' ||
      error.details?.reason === 'control_detached_before_inspection');
}
