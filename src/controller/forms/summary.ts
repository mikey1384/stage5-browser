...
export const formSummaryOperations = {
  async formSummary(input: BrowserCommandInput<'formSummary'>): Promise<BrowserCommandOutput<'formSummary'>> {
    ...
    const candidates = root.locator.locator(FIELD_SELECTOR);
    ...
    try {
      for (let index = 0; index < Math.min(totalFields, input.maxFields); index += 1) {
        ...
        const descriptor = await boundedValue(
          handle.evaluate((element) => {
            ...
            if (ariaInvalid !== null) valid = ariaInvalid !== 'true';
            else if ('checkValidity' in element && typeof element.checkValidity === 'function') valid = element.checkValidity();
            const optionNames = element instanceof HTMLSelectElement
              ? Array.from(element.options).slice(0, 100).map((option) => normalize(option.label || option.textContent || '')).filter(Boolean)
              : [];
            return {
              kind,
              role,
              ...
              disabled,
              readOnly,
              multiple: element instanceof HTMLSelectElement ? element.multiple : element.getAttribute('aria-multiselectable') === 'true',
              optionNames,
              optionsComplete: !(element instanceof HTMLSelectElement) || element.options.length <= 100,
              valuePresence,
              selected,
              valid,
            } satisfies Omit<FormFieldObservation, 'fieldId'>;
          }),
          Math.max(1, remainingUntil(deadlineAt)),
          null,
        );
        ...
        fields.set(fieldId, { locator, handle, observation: { fieldId, ...descriptor } });
      }
      ...
      if (!retained) {
        await Promise.allSettled([...fields.values()].map(({ handle }) => handle.dispose()));
      }
    }
  },
