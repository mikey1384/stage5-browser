...
function createServer(): McpServer {
  ...
  server.registerTool(
    'browser_resume_after_login',
    {
      ...
      description:
        'After the user closes the private native browser normally, verify clean shutdown plus exact executable/profile and privacy-safe offline storage evidence, reject origin-only auth URL expectations, reattach Playwright, remove the Stage5 marker tab, and return a bounded semantic preview. Returns AUTH_NOT_PERSISTED when the human phase added target-origin session metadata but a supplied non-root post-login route cannot be reached. Cookie values are never inspected, and live Chromium SQLite presence is treated as unknown. Visible site state still requires a fresh full semantic snapshot.',
      inputSchema: z.object({
