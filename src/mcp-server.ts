...
function createServer(): McpServer {
  ...
  server.registerTool(
    'browser_inspect_tab',
    {
      title: 'Inspect exact tab without activation',
      description: 'Capture one ref-free semantic document view from the exact session-scoped opaque tabId returned by browser_tabs without selecting the tab, bringing it forward, or exposing element/frame action capabilities. This is read-only evidence for duplicate or background tabs. It never falls back to URL, title, or index; stale identities fail closed. A visible modal may suppress underlying application content and is reported explicitly—never close or dismiss preserved state merely to expose it.',
      inputSchema: z.object({
        tabId: tabIdSchema,
