...
function createServer(): McpServer {
  ...
  server.registerTool(
    'browser_click_ref',
    {
      ...
      description:
        'Click one Playwright reference from the latest semantic snapshot of the same document and frame. A fresh offscreen ref receives bounded incremental nested/document scrolling while retaining the exact DOM node. If feed virtualization detaches it, only one uniquely proven same-article semantic replacement may be rebound before actionability revalidation; preparation failures report actionDispatched false. Stale, unknown, reused, changed, or ambiguous references fail closed. Optional postconditions verify the requested state change; failures record sanitized actionability evidence.',
