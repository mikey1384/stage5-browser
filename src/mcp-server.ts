...
function createServer(): McpServer {
  ...
  server.registerTool(
    'browser_auth_status',
    {
      ...
      description:
        'Report the dedicated persistent profile, exact executable/profile binding, native application and handoff label, control mode, private native-process state, unambiguous clean-shutdown evidence, sanitized boundary/storage continuity, and verification lifecycle. Authentication remains unknown until an agent verifies site-specific signed-in state.',
      inputSchema: z.object({}),
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
  ...
  server.registerTool(
    'browser_resume_after_login',
    {
      ...
      description:
        'After the private native browser exits, require zero profile locks and the exact executable/profile identity. A zero process exit with no signal permits reattachment even if Chromium retains a stale crashed marker; marker value and modification time are compared with the pre-handoff snapshot. An abnormal or unavailable exit offers one explicit second-call override only while the process is gone and locks are clear. Resume also checks privacy-safe offline storage evidence, rejects origin-only auth URL expectations, removes the Stage5 marker tab, and returns a bounded semantic preview. Returns AUTH_NOT_PERSISTED when the human phase added target-origin session metadata but a supplied non-root post-login route cannot be reached. Cookie values are never inspected, and live Chromium SQLite presence is treated as unknown. Visible site state still requires a fresh full semantic snapshot.',
      inputSchema: z.object({
