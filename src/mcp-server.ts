...
function createServer(): McpServer {
  ...
  server.registerTool(
    'browser_set_input_files',
    {
      ...
      description:
        'Transfer one or more explicitly authorized regular local files into a file input observed in the latest semantic snapshot. Uses a document-bound one-use file-input ref, rejects relative paths, symlinks, directories, stale refs, disabled controls, and unsupported multiple selection, and never opens a native picker. Returns privacy-minimized file metadata, a fresh attachment preview, semantic progress evidence, temporally bounded network error counts, and an explicit unverified state when processing completion cannot be proven. This may immediately start an external upload; never replay it after an ambiguous failure.',
  ...
  server.registerTool(
    'browser_request_login_handoff',
    {
      ...
      description:
        'Optionally navigate the controlled isolated profile to a login URL, close current control cleanly, and launch the same exact executable/profile binding as a native browser without Playwright automation flags. A static Stage5 marker tab plus the returned real application name and short label identify the correct window. The user authenticates privately while browser tools remain unavailable. Follow the returned backend-specific instruction: leave Chromium-family browsers open for same-process attachment; quit Firefox normally for restart-based resume. Credentials, passkeys, CAPTCHAs, and OTPs stay out of agent messages and logs.',
      inputSchema: z.object({
  ...
  server.registerTool(
    'browser_resume_after_login',
    {
      ...
      description:
        'Resume the backend-specific private login handoff. Chromium-family browsers must still be running: Stage5 attaches to that exact process over its private loopback channel, so session cookies never cross a restart boundary and compatible worker reloads can reconnect. Firefox retains the normal-exit, unlocked-profile restart path. Resume rejects origin-only auth URL expectations, verifies exact executable/profile identity and the actual Chromium runtime profile, compares privacy-safe target-origin storage continuity, removes the Stage5 marker, and returns a bounded semantic preview. Returns AUTH_NOT_PERSISTED on verified loss or when human-added metadata cannot reach a supplied non-root post-login route. Cookie values and exact human interactions are never inspected. Visible site state still requires a fresh full semantic snapshot.',
      inputSchema: z.object({
