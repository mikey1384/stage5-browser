...
function createServer(): McpServer {
  ...
  server.registerTool(
    'browser_inspect_tab',
    {
      ...
      description: 'Capture one ref-free semantic document view from the exact session-scoped opaque tabId returned by browser_tabs without exposing element/frame action capabilities. The default is strictly passive and never selects or activates the tab. For a hidden renderer whose dynamic content cannot advance, temporaryActivation=true may bring only that exact tab forward inside the already controlled browser, optionally wait for bounded generic article-growth/loading-disappearance evidence, then restore and prove the prior exact controller-selected tab before returning. It does not call Stage5\'s native application-activation path or change the controller selection, and it never falls back to URL, title, or index. A stale identity or unproven restoration fails closed. A visible modal may suppress underlying application content and is reported explicitly—never close or dismiss preserved state merely to expose it.',
      inputSchema: z.object({
        tabId: tabIdSchema,
