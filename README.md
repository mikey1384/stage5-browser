...
| `browser_click_ref` | Incrementally prepare one fresh reference, uniquely rebind feed virtualization, and instrument a guarded exact-target pointer dispatch |
...
- Exact-ref clicks install a bounded capture guard and record only connectivity, geometry-change, and trusted pointer/mouse/click booleans. A normal stable-click timeout may use one forced exact-handle fallback only when no trusted event was emitted and the same node remains fully actionable. Misdirected or newly non-actionable input is blocked; any partial or uncertain dispatch stops without another click.
