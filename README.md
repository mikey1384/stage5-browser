...
| `browser_open` | Navigate with bounded commit, readiness, client-redirect stabilization, and explicit acknowledgement of known unsaved attachment state for same-page navigation |
...
| `browser_click_by_role` | Resolve one unique role/name target and use the shared deadline-safe exact-target engine, optionally verifying URL, selected, visible, exact hidden state, or acknowledging known navigation risk |
| `browser_click_ref` | Incrementally prepare one fresh reference, uniquely rebind feed virtualization, then use the same deadline-safe exact-target engine, postconditions, and agent-declared navigation-risk gate |
...
| `browser_set_input_files` | Select authorized regular local files through a fresh file-input ref and distinguish processing completion from unproven workflow persistence |
...
| `browser_inspect_control` | Inspect one exact control and bounded options with an agent-chosen pointer/keyboard reveal; focus on or inside the exact semantic target and expanded evidence are evaluated before global owner inventory without overriding a competing structural owner |
| `browser_select_option` | Set one exact native/custom option; in auto mode, direct or inspected options on an editable search control atomically type the exact label and Enter on that control, then prove selected state |
| `browser_select_options` | Select exact multi-select options with an agent-chosen pointer/keyboard opener reveal, refreshing capabilities between physical inputs and preserving completed evidence |
...
- `docs/dogfooding-2026-08-27-composite-focus-popup-ownership.md` — contained-focus ownership, structural-conflict precedence, and categorical target-first miss telemetry in 0.21.2
- `docs/dogfooding-2026-08-27-searchable-option-commit-routing.md` — inspected-search auto routing, pointer-only exact option clicks, and categorical transport telemetry in 0.21.2
- `docs/dogfooding-2026-08-27-multi-select-reveal-routing.md` — direct multi-selection opener-transport choice, delayed-effect stabilization, and fresh-host telemetry in 0.21.4
- `docs/dogfooding-2026-08-27-unsaved-attachment-navigation.md` — page-scoped unsaved attachment state, agent-discretion navigation acknowledgement, native reattach continuity, and categorical telemetry in 0.21.4
...
- `docs/dogfooding-2026-08-27-desired-option-state.md` — same-field composite-chip mapping plus exact selected/unselected control motion and directional telemetry in 0.16.0
- `docs/dogfooding-2026-08-27-covered-option-contact.md` — bounded scroll-surface interior positioning when a newly visible exact option remains covered in 0.16.1
- `docs/dogfooding-2026-08-27-composite-popup-surfaces.md` — one-owner composite popup surfaces and no-replay passive recovery in 0.16.2
...
- Control inspection discovers rendered popup surfaces through one bounded source of truth. Semantic list/menu/tree roles remain primary; a generic positioned, popover, or dialog option group is accepted only when it proves a portal boundary, and its innermost real scroll surface is retained. Strict option branches separated by a real bounded visual gap under one broad positioned portal are partitioned before ownership, while contiguous wrappers remain one surface. Association remains explicit/structural first, then focused, expanded, or spatial ownership. Nested owned surfaces collapse to their outer root; disjoint panels remain one composite capability only when every surface belongs to the same exact owner. Multiple non-structural candidates resolve only when one has a decisive normalized edge-gap lead. In the spatial tier, a material overlap whose bounded hit test proves the popup covers that sibling cannot tie one tightly adjacent exterior anchor; stronger evidence, uncovered overlap, different owners, and exterior ties remain closed. Results and traces expose only categorical association/surface/owner proof and bounded counts.
