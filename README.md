...
- `expectedHidden` proves that one exact semantic role/name became absent from the accessible surface or uniquely hidden. A remaining visible match fails, and duplicate matches, frame loss, or observation failure stay unknown rather than being mistaken for hidden. The result reports only `expected: false` and `observed: false | true | null`, never the supplied name. This is an effect boundary, not proof of a persisted field value or later submission.
...
- Lounge join/send/wait/ack/status bypass an unrelated browser-contract freshness gate, so active coordination stays wakeable while browser operations wait for the required reconnect.
