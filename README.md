...
- Control inspection discovers rendered popup surfaces through one bounded source of truth. Semantic list/menu/tree roles remain primary; a generic positioned, popover, or dialog option group is accepted only when it proves a portal boundary, and its innermost real scroll surface is retained. Strict option branches separated by a real bounded visual gap under one broad positioned portal are partitioned before ownership, while contiguous wrappers remain one surface. Association remains explicit/structural first, then unique focused, expanded, or spatial ownership; unresolved surfaces or owners fail before input. Results and traces expose only categorical association/surface proof and bounded counts.
