...
- Unnamed textboxes and contenteditables can be filled only through a fresh snapshot-bound `browser_fill_ref`. It revalidates the exact document, frame, and modal scope, consumes the ref once, never journals or returns the supplied value, and reports only input/change events plus exact value-match booleans.
