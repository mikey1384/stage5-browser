...
- `docs/dogfooding-2026-08-25-facebook-fill-preparation.md` — snapshot-captured editor/scope capabilities, exact preparation-step evidence, and truthful pre-keyboard detachment classification fixed in 0.7.2
...
- Unnamed textboxes and contenteditables can be filled only through a fresh snapshot-bound `browser_fill_ref`. It revalidates the exact document, frame, and modal scope, pins the exact observed handle through dispatch, consumes the ref once, never journals or returns the supplied value, and reports the bounded fill phase, target state, input/change events, and exact logical value-match booleans.
