...
import { processIsRunning, readNativeControlRecord } from '../src/native-control-channel.js';
import {
...
describe('BrowserController', () => {
  ...
  it('retains privacy-safe last-action evidence for the exact native target across worker replacement', async () => {
    ...
    const port = await listen(server);
    temporaryRoot = await mkdtemp(path.join(os.tmpdir(), 'stage5-browser-retained-action-'));
    ...
    (controller as unknown as { nativeControlRecord: NativeControlRecord | null }).nativeControlRecord = nativeRecord;
    vi.spyOn(
      controller as unknown as { chromiumTargetId: (candidate: Page) => Promise<string | null> },
      'chromiumTargetId',
    ).mockResolvedValue(selectedTargetId);
    vi.spyOn(
      controller as unknown as { chromiumDocumentId: (candidate: Page) => Promise<string | null> },
      'chromiumDocumentId',
    ).mockResolvedValue(documentId);

    await controller.persistActionDiagnosticsForWorkerHandoff();
    const persisted = await readNativeControlRecord(config.profileDir, 'chromium');
    ...
    if (persisted?.retainedAction === undefined) {
      throw new Error('The retained action fixture did not persist exact-target evidence.');
    }

    (controller as unknown as { pageDiagnostics: PageDiagnosticBuffer }).pageDiagnostics = new PageDiagnosticBuffer();
    (controller as unknown as { nativeControlRecord: NativeControlRecord | null }).nativeControlRecord = {
      ...persisted,
      retainedAction: {
    ...
    await (controller as unknown as { restoreNativeActionAfterAttach: (candidate: Page) => Promise<void> })
      .restoreNativeActionAfterAttach(page);
    expect((await controller.diagnostics()).page?.lastAction).toBeNull();

    (controller as unknown as { nativeControlRecord: NativeControlRecord | null }).nativeControlRecord = persisted;
    await (controller as unknown as { restoreNativeActionAfterAttach: (candidate: Page) => Promise<void> })
      .restoreNativeActionAfterAttach(page);
    expect((await controller.diagnostics()).page?.lastAction).toMatchObject({
      action: 'click_by_role',
  ...
  it('proves one exact option became hidden without returning its semantic name', async () => {
    ...
    const port = await listen(server);
    temporaryRoot = await mkdtemp(path.join(os.tmpdir(), 'stage5-browser-hidden-option-'));
    controller = new BrowserController(browserConfig(temporaryRoot));
    await controller.open({
    ...
    const observed = await controller.snapshot({ depth: 6, boxes: false, frameId: null, timeoutMs: 2_000 });
    const optionRef = observed.snapshot.match(/option "Operating revenue"[^\n]*\[ref=([^\]]+)\]/)?.[1];
    expect(optionRef).toBeDefined();
    if (optionRef === undefined) throw new Error('Option fixture did not expose its exact ref.');
    ...
    expect(JSON.stringify(result.postcondition)).not.toContain('Operating revenue');
    const page = (controller as unknown as { activePage: Page }).activePage;
  ...
  it('accepts exact-option disappearance as terminal success after partial pointer input without replay', async () => {
    ...
    const port = await listen(server);
    temporaryRoot = await mkdtemp(path.join(os.tmpdir(), 'stage5-browser-partial-hidden-option-'));
    controller = new BrowserController(browserConfig(temporaryRoot));
    await controller.open({
      url: `http://127.0.0.1:${port}/choice`,
      newTab: false,
    ...
    const optionRef = observed.snapshot.match(/option "Operating revenue"[^\n]*\[ref=([^\]]+)\]/)?.[1];
    expect(optionRef).toBeDefined();
    if (optionRef === undefined) throw new Error('Partial option fixture did not expose its exact ref.');
    const dispatch = vi.spyOn(
      controller as unknown as { dispatchExactHandleClick: (...args: unknown[]) => Promise<void> },
      'dispatchExactHandleClick',
    );
    ...
    const page = (controller as unknown as { activePage: Page }).activePage;
    await expect(page.locator('#counters').textContent()).resolves
      .toBe('pointerdowns:1 mousedowns:1 clicks:0');
  });
  ...
  it('fails closed after one confirmed option click when the exact option remains visible', async () => {
    ...
    const port = await listen(server);
    temporaryRoot = await mkdtemp(path.join(os.tmpdir(), 'stage5-browser-unmet-hidden-option-'));
    controller = new BrowserController(browserConfig(temporaryRoot));
    await controller.open({
      url: `http://127.0.0.1:${port}/choice`,
      newTab: false,
    ...
    await expect(controller.clickByRole({
      ...
      details: {
        ...
        checks: [{ kind: 'visible', passed: false, expected: false, observed: true }],
  ...
  it('does not treat an ambiguous post-click option set as hidden', async () => {
    ...
    const port = await listen(server);
    temporaryRoot = await mkdtemp(path.join(os.tmpdir(), 'stage5-browser-ambiguous-hidden-option-'));
    controller = new BrowserController(browserConfig(temporaryRoot));
    await controller.open({
      url: `http://127.0.0.1:${port}/choice`,
      newTab: false,
    ...
    await expect(controller.clickByRole({
      ...
      details: {
        clickDispatched: true,
        checks: [{ kind: 'visible', passed: false, expected: false, observed: null }],
      },
    });
    const page = (controller as unknown as { activePage: Page }).activePage;
    await expect(page.locator('#counters').textContent()).resolves.toBe('clicks:1');
    await expect(page.getByRole('option', { name: 'Operating revenue', exact: true }).count()).resolves.toBe(2);
  });
